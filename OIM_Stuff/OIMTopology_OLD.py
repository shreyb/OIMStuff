...
class ResourceGroup:
    ...
    is pulled from OIM"""
    def __init__(self, rgname, rgdescription):
        """
        Args:
...
class Resource:
    ...
    is pulled from OIM"""
    def __init__(self, rname, rid, fqdns):
        """
        Args:
    ...
    def parse(self):
        ...
        for resource_group_element in d.getElementsByTagName("ResourceGroup"):
            if resource_group_element.getElementsByTagName("Disable")[0].childNodes[0].data.strip() == "True":
                continue
            site = resource_group_element.getElementsByTagName("GroupName")[0].childNodes[0].data.strip()
            description = site
            resource_group = ResourceGroup(site, description)
        ...
        for r in self.document.getElementsByTagName("Resource"):
            ...
            for s in r.getElementsByTagName("Service"):
                if s.getElementsByTagName("Name")[0].childNodes[0].data.strip() == "CE"\
                    ...
                    fqdn = r.getElementsByTagName("FQDN")[0].childNodes[0].data.strip()
                    self.resources[rid] = Resource(name, rid, fqdn)
                    break
